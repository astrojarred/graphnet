...
class CrossAttentionLayerDir(torch.nn.Module):
...
class DynEdgeStereoDir(GNN):
    ...
    def __init__(
        ...
        *,
        ...
        # Default to GELU for better direction reconstruction performance
        if self._activation_type is None or self._activation_type.lower() == "gelu":
            self._activation = torch.nn.GELU()
        elif self._activation_type.lower() == "relu":
            self._activation = torch.nn.ReLU()
        elif self._activation_type.lower() == "leakyrelu":
            self._activation = torch.nn.LeakyReLU()
        else:
            raise ValueError(f"Activation layer {self._activation_type} not supported. Supported: gelu, relu, leakyrelu")
