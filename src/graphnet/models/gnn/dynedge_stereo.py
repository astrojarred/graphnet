...
__all__ = ["DynEdgeStereo", "CrossAttentionLayer"]
...
class DynEdgeStereo(GNN):
    ...
    def _calculate_stereo_global_variables(
        ...
        # Determine the number of graphs (events) in the original batch
        dim_size = original_data.batch.max().item() + 1
        
        # Basic global variables for each telescope
        tel1_globals = self._calculate_telescope_global_variables(tel1_data, dim_size, device)
        tel2_globals = self._calculate_telescope_global_variables(tel2_data, dim_size, device)
        ...
        if self._add_stereo_global_features:
            ...
            tel1_mean_time = scatter_mean(tel1_data.x[:, 2], tel1_data.batch, dim=0, dim_size=dim_size)  # Assuming t is at index 2
            tel2_mean_time = scatter_mean(tel2_data.x[:, 2], tel2_data.batch, dim=0, dim_size=dim_size)
            time_diff = tel1_mean_time - tel2_mean_time
            
            # Signal ratios
            tel1_total_signal = scatter_sum(tel1_data.x[:, -3], tel1_data.batch, dim=0, dim_size=dim_size)  # Assuming signal is at index -3
            tel2_total_signal = scatter_sum(tel2_data.x[:, -3], tel2_data.batch, dim=0, dim_size=dim_size)
            ...
            tel1_pointing = scatter_mean(tel1_data.x[:, -2:], tel1_data.batch, dim=0, dim_size=dim_size)
            tel2_pointing = scatter_mean(tel2_data.x[:, -2:], tel2_data.batch, dim=0, dim_size=dim_size)
    ...
    def _calculate_telescope_global_variables(self, data: Data, dim_size: int, target_device: Optional[torch.device] = None) -> Tensor:
        ...
        if x is None or x.shape[0] == 0 or batch is None or batch.numel() == 0:
            ...
            if target_device is not None:
                ...
                expected_features = self._effective_nb_inputs + 4
                ...
                expected_features = self._effective_nb_inputs + 4
            # Return zeros with shape [dim_size, expected_features]
            return torch.zeros(dim_size, expected_features, device=device)
        ...
        # Calculate mean features with aligned dim_size
        global_means = scatter_mean(x, batch, dim=0, dim_size=dim_size)
        ...
        if edge_index is not None and edge_index.shape[1] > 0:
            ...
            if h_t.dim() > 2:
                ...
                if h_t.dim() == 1:
                    h_t = h_t.unsqueeze(1)
            # Right-pad homophily outputs to dim_size if needed
            def _right_pad(t: Tensor, rows: int) -> Tensor:
                if t.shape[0] < rows:
                    ...
                    return t[:rows]
                return t
            h_x = _right_pad(h_x, dim_size)
            h_y = _right_pad(h_y, dim_size)
            h_z = _right_pad(h_z, dim_size)
            h_t = _right_pad(h_t, dim_size)
                    
        else:
            # No edges - create zero homophily
            # Safe batch size calculation
            h_x = torch.zeros(dim_size, 1, device=device)
            h_y = torch.zeros(dim_size, 1, device=device)
            h_z = torch.zeros(dim_size, 1, device=device)
            h_t = torch.zeros(dim_size, 1, device=device)
        
        # Combine global variables
        # global_means: [batch_size, nb_features]
        # h_x, h_y, h_z, h_t: [batch_size, 1] each
        global_variables = torch.cat([
            global_means,    # [dim_size, nb_features]
            h_x, h_y, h_z, h_t  # [dim_size, 1] each
        ], dim=1)
        
        return global_variables
        
    def _global_pooling(self, x: Tensor, batch: LongTensor, dim_size: Optional[int] = None) -> Tensor:
        """Perform global pooling."""
        pooled = []
        for pooling_scheme in self._global_pooling_schemes:
            pooling_fn = GLOBAL_POOLINGS[pooling_scheme]
            if dim_size is not None:
                pooled_x = pooling_fn(x, index=batch, dim=0, dim_size=dim_size)
            else:
    ...
    def forward(self, data: Data) -> Tensor:
        ...
        # Global pooling for each telescope separately (aligned to original batch size)
        tel1_pooled = self._global_pooling(tel1_combined, tel1_data.batch, dim_size=dim_size)
        tel2_pooled = self._global_pooling(tel2_combined, tel2_data.batch, dim_size=dim_size)
