...
def main():
    ...
    if args.wandb and not args.eval_only:  # Skip W&B for eval-only mode
        ...
        hyperparams = {
            ...
            "learning_rate": 1e-3,  # from optimizer_kwargs in create_model()
            "optimizer": "Adam",
            "optimizer_eps": 1e-08,  # from optimizer_kwargs in create_model()
            "nb_nearest_neighbours": 8,  # from KNNGraph in create_model()
            "global_pooling_schemes": ["min", "max", "mean", "sum"],
            "graph_columns": [0, 1, 2],  # x_cam, y_cam, t
            ...
            "energy_loss_weight": "particle_id",  # Only for gamma events
            "direction_loss_weight": "particle_id",  # Only for gamma events
            "num_tasks": 3,
            
            # Data and features
            "features": list(features) if hasattr(features, '__iter__') else str(features),
            "truth_labels": list(truth + ["particle_id"]),
            "detector": "MAGIC",
            "dataset_type": "LMDB",
            ...
            "backbone": "DynEdge",
            "graph_definition": "KNNGraph",
            "detector_class": "MAGICDetector",
            
            # Task-specific details
            "energy_activation": "softplus_beta_0.1",  # From MAGICEnergyReconstruction
            "direction_outputs": 2,  # theta, phi
        }
        
        # Add any additional arguments that might be relevant
        for arg_name, arg_value in vars(args).items():
            if arg_name not in hyperparams and arg_value is not None:
                hyperparams[f"arg_{arg_name}"] = arg_value
        ...
        wandb_kwargs = {
            ...
            "save_dir": wandb_dir,
            "log_model": True,
            "config": hyperparams,  # Log all hyperparameters
        }
        ...
        # Log additional hyperparameters after logger creation (in case of resuming)
        if not wandb_run_id:  # Only log if this is a new run
            logger.info(f"📊 Logging {len(hyperparams)} hyperparameters to W&B")
    ...
    if args.eval_only:
        ...
        logger.info("🎯 Tasks: Classification + Energy + Direction (3 tasks total)")
