"""Training script for MAGIC energy reconstruction.

This script trains models for MAGIC energy reconstruction using energy-weighted loss.
"""
...
# Import energy reconstruction components
from graphnet.models.task.magic_energy_reco import MAGICEnergyRecoValidationLogger

# -----------------------------------------------------------------------------
# Logging callbacks adapted for energy reconstruction
# -----------------------------------------------------------------------------
...
class SmartSWACallback(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        self.swa_callback = StochasticWeightAveraging(
            swa_lrs=self.scheduler_final_lr, swa_epoch_start=self.swa_epoch_start
        )
        ...
        logger = Logger()
        logger.info(
            f"SmartSWA: Using scheduler final LR {self.scheduler_final_lr:.2e} for SWA"
        )

    def _estimate_final_lr_from_model(self, pl_module):
        """Estimate final LR from model's scheduler configuration."""
        try:
            if hasattr(pl_module, "_scheduler_class") and hasattr(
                pl_module, "_scheduler_kwargs"
            ):
                scheduler_class = pl_module._scheduler_class
                scheduler_kwargs = pl_module._scheduler_kwargs or {}
                base_lr = (
                    pl_module._optimizer_kwargs.get("lr", 1e-4)
                    if hasattr(pl_module, "_optimizer_kwargs")
                    else 1e-4
                )
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        if self.swa_callback and hasattr(self.swa_callback, "on_train_epoch_start"):
            if (
                trainer.current_epoch >= self.swa_epoch_start
                and self.swa_callback._average_model is not None
            ):
...
def main(
    ...
    num_workers: int = 10,
    wandb: bool = False,
    wandb_project: str = "magic-energy-reco",
    ...
    """Train MAGIC energy reconstruction model.
    ...
    test_dataset = EnsembleDataset([datasets[key] for key in datasets if "test" in key])
    ...
    training_dataloader = DataLoader(train_dataset, shuffle=True, **dataloader_config)
    ...
    def _safe_get_energy(data_item):
        value = getattr(data_item, energy_key, None)
        if value is None:
            return None
        return value

    energy_batch1 = _safe_get_energy(data1)
    energy_batch2 = _safe_get_energy(data2)
    if energy_batch1 is not None and energy_batch2 is not None:
        print(f"Batch 1 energy mean: {energy_batch1.mean():.6f}")
        print(f"Batch 2 energy mean: {energy_batch2.mean():.6f}")
        print(f"Same data? {torch.allclose(energy_batch1, energy_batch2)}")
    else:
        print(f"Energy key '{energy_key}' not found in validation batches.")
    ...
    logger.info(
        f"Model loss functions: {[type(getattr(task, '_loss_function', None)).__name__ for task in model._tasks]}"
    )
    ...
    if use_swa:
        if swa_lrs is not None:
            ...
            swa_callback = StochasticWeightAveraging(
                swa_lrs=swa_lrs, swa_epoch_start=swa_epoch_start
            )
        else:
            logger.info(
                f"SWA: Starting epoch {swa_epoch_start}, using scheduler's final LR"
            )
            swa_callback = SmartSWACallback(
                swa_epoch_start=swa_epoch_start, fallback_lr=1e-4
            )
    ...
    callbacks.append(
        MAGICEnergyRecoValidationLogger(
            log_every_n_batches=100,
            accumulate_over_epoch=True,
        )
    )

    callbacks.append(TrainLossLogger(log_every_n_batches=50))
    ...
    if profiler:
        ...
        if profiler == "simple":
            profiler_obj = SimpleProfiler(dirpath=output_dir, filename="profile_simple")
            logger.info(
                f"Using SimpleProfiler, output will be saved to {output_dir}/profile_simple.txt"
            )
        elif profiler == "advanced":
            profiler_obj = AdvancedProfiler(
                dirpath=output_dir, filename="profile_advanced"
            )
            logger.info(
                f"Using AdvancedProfiler, output will be saved to {output_dir}/profile_advanced.txt"
            )
        elif profiler == "pytorch":
            profiler_obj = PyTorchProfiler(
                dirpath=output_dir, filename="profile_pytorch", export_to_chrome=True
            )
            logger.info(
                f"Using PyTorchProfiler, output will be saved to {output_dir}/profile_pytorch.json"
            )
        else:
            logger.warning(
                f"Unknown profiler '{profiler}', falling back to string argument"
            )
    ...
    # Train the model
    logger.info("Starting energy reconstruction training")
    ...
    # Save state dict (safer, do this first)
    ...
    try:
        ...
        logger.warning(f"Failed to save full model object: {e}")
        logger.warning(
            "This is likely due to an unpickleable object in the model (e.g. profiler)."
        )
        logger.warning(
            "The state_dict was saved successfully, so you can still load the weights."
        )
    ...
    try:
        ...
        if all(col in results.columns for col in ["energy_pred", energy_key]):
            pred_energy = torch.tensor(results["energy_pred"].values)
            true_energy = torch.tensor(results[energy_key].values)

            # Energy metrics
            energy_errors = torch.abs(pred_energy - true_energy)
            median_energy_error = torch.median(energy_errors).item()
            mean_energy_error = torch.mean(energy_errors).item()
            energy_error_68 = torch.quantile(energy_errors, 0.68).item()
            energy_error_95 = torch.quantile(energy_errors, 0.95).item()

            logger.info("Energy Performance:")
            logger.info(f"  Median error: {median_energy_error:.4f} GeV")
            logger.info(f"  Mean error: {mean_energy_error:.4f} GeV")
            logger.info(f"  68% containment: {energy_error_68:.4f} GeV")
            logger.info(f"  95% containment: {energy_error_95:.4f} GeV")

            # Save metrics
            metrics = {
                "median_energy_error": median_energy_error,
                "mean_energy_error": mean_energy_error,
                "energy_error_68": energy_error_68,
                "energy_error_95": energy_error_95,
                "approach": "energy_reco",
                "use_uncertainty": use_uncertainty,
            }
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Train MAGIC Energy reconstruction model")
    ...
    # Energy reconstruction specific arguments
    parser.add_argument(
        "--use-uncertainty",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="PyTorch Lightning profiler to use (e.g., 'simple', 'advanced', 'pytorch')",
    )
